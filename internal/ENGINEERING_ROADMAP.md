...
Last refreshed: `2026-04-06` after update-cache work, startup auto-check, diagnostics, packages/archive improvements, cold update-check concurrency, and five layout/build extraction passes
...
- Status: first extraction passes done via dedicated setup workspace surface, inventory controls tabs, and packages, compare, and discovery workspace builders inside `MainWindow`; deeper `__init__` and `_build_layout` decomposition is still pending.
...
- the first setup-surface, inventory-controls, and packages/compare/discovery workspace extraction passes are landed
- `MainWindow.__init__` and `_build_layout` remain the largest concentrated UI setup seams
- the next anti-drift win is another coherent page/control builder extraction, likely around archive/install/recovery workspace shells, before starting another product wave
